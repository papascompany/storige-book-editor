 ...
 *    v1 무인증 라우트 0 원칙).
 *  - Guards: PartnerApiKeyGuard(Bearer/X-API-Key 병행 인증)
 *  - Filter: PartnerApiExceptionFilter(에러 봉투 §3.2 + 에러 감사)
 *  - Interceptors(바깥→안): 감사 → 성공 봉투(§3.1)
 *
...
export function PartnerV1Controller(path = ''): ClassDecorator {
  return applyDecorators(
    ...
    UseInterceptors(PartnerAuditInterceptor, PartnerEnvelopeInterceptor),
  );
}
