 ...
 *
 * ── "전체 병렬 26s 타임아웃" 플레이크 해소 기록 (2026-08-26) ─────────────────
 * 이 spec 은 여러 스프린트 동안 "단독 PASS·전체 병렬 간헐 타임아웃"으로 플레이크
 * 등재돼 있었다. 원인은 이 파일이 아니다 — supertest 가 IPv6 '::' 로 받은 포트에
 * IPv4 127.0.0.1 로 접속해, 같은 포트의 IPv4 쪽을 점유한 로컬 상주 데몬으로 요청이
 * 새던 주소 패밀리 버그(0d65984 에서 test/setup.ts serverAddress [::1] 재작성으로
 * 근본 수정)의 변종이다: Go 데몬(즉시 4xx)에 걸리면 403/404 플레이크, "accept 후
 * 무응답" 침묵 리스너(실측: ephemeral 대역 상주 2개)에 걸리면 응답이 영원히 안 와
 * jest testTimeout(30s)으로 발현됐다.
 *
 * 유력 가설이던 "해싱 고비용 라운드"는 코드로 반증 — 파트너 키 해시는 SHA-256
 * 단일 digest(라운드 개념 없음, μs 급)이고 SitesService 는 stub 이라 이 경로에
 * bcrypt 호출이 0회다. 프로덕션 보안 파라미터는 아무것도 바꾸지 않았다.
 * 실측(2026-08-26): 단독 2.68s·20개 테스트 전부 ≤19ms, CPU busy-loop 10개
 * (hw.ncpu=10 전점유) 하에서도 3.58s — 30s 를 만들 내부 구성요소가 없다.
 * setup.ts 수정 후 전체 병렬 46회 연속 무실패. → 이 spec 을 다시 플레이크로
 * 등재하지 말 것. 재발 시 spec 내부가 아니라 로컬 포트 스쿼터부터 의심하라.
 */
import {
  ExecutionContext,
  Get,
