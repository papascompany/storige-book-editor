...
describe('파트너 포털 v0 (/api/portal/sites/:siteId — SITE_ADMIN 셀프서브)', () => {
  ...
  describe('인가 경계 (크로스 테넌트/역할)', () => {
    ...
    it('SITE_MANAGER — RolesGuard 에서 403 (포털은 SITE_ADMIN 전용)', async () => {
      await as(siteManager).get(`/api/portal/sites/${SITE_A}`).expect(403);
    });
  ...
  describe('가드 스택 계약 (리플렉션)', () => {
    ...
    it('@Roles(SITE_ADMIN) 단독 — 전역 ADMIN/MANAGER 부여 금지(기존 화면 무변경 계약)', () => {
      const roles = Reflect.getMetadata(ROLES_KEY, PortalController);
      expect(roles).toEqual([UserRole.SITE_ADMIN]);
    });
