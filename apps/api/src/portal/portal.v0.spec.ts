...
describe('파트너 포털 v0 (/api/portal/sites/:siteId — SITE_ADMIN 셀프서브)', () => {
  ...
  describe('test 키 셀프 발급/목록/폐기', () => {
    ...
    it('사이트당 활성 test 키 상한 도달 시 발급 429 PORTAL_TEST_KEY_LIMIT', async () => {
      const prev = process.env.PORTAL_TEST_KEY_LIMIT;
      process.env.PORTAL_TEST_KEY_LIMIT = '2';
      try {
        // 상한(2)까지 활성 test 키 시드
        await keysService.issue(SITE_A, 'test', 'k1');
        await keysService.issue(SITE_A, 'test', 'k2');
        // 3번째 발급 거부
        const res = await as(siteAdminA)
          .post(`/api/portal/sites/${SITE_A}/partner-keys`)
          .send({ name: 'over-limit' })
          .expect(429);
        expect(res.body.code).toBe('PORTAL_TEST_KEY_LIMIT');
        // 발급이 실제로 일어나지 않았는지(활성 test 키 여전히 2건)
        expect(
          keyRepo.rows.filter(
            (r) => r.siteId === SITE_A && r.env === 'test' && r.status === 'active',
          ),
        ).toHaveLength(2);

        // revoked 키는 상한에서 제외 — 하나 폐기하면 재발급 가능
        const toRevoke = keyRepo.rows.find(
          (r) => r.siteId === SITE_A && r.env === 'test',
        )!;
        await as(siteAdminA)
          .delete(`/api/portal/sites/${SITE_A}/partner-keys/${toRevoke.id}`)
          .expect(200);
        await as(siteAdminA)
          .post(`/api/portal/sites/${SITE_A}/partner-keys`)
          .send({ name: 'after-revoke' })
          .expect(201);
      } finally {
        if (prev === undefined) delete process.env.PORTAL_TEST_KEY_LIMIT;
        else process.env.PORTAL_TEST_KEY_LIMIT = prev;
      }
    });
