 ...
 *  ② RolesGuard + @Roles(SITE_ADMIN) — 전역 ADMIN/MANAGER 는 이 표면 밖(기존
 *     /api/sites·/api/sites/:id/partner-keys 사용). SUPER_ADMIN 은 가드 무조건 통과.
 *  ③ TenantGuard — :siteId 가 JWT siteRoles 멤버십에 포함되는지(크로스 테넌트 403)
 *  ④ (서비스) assertSiteAdmin — 해당 site 의 per-site 역할이 SITE_ADMIN 인지
 *     (site A=ADMIN, site B=MANAGER 혼합 배정의 역할 경계)
 *
 * 응답 규약:
