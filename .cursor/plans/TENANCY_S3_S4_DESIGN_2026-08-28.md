...
### 2-B'. NULL 파일 백필 (D6 선행 검토 — 2026-08-28 bookmoa 질의로 등재)

기존 무귀속 파일의 site 를 **세션 역참조**로 소급 도출한다(자동화 가능 범위만):
`files ← file_edit_sessions(cover/content/contentPdf_file_id) → session.siteId`.
실측(2026-08-28, live NULL 225건): 역참조 귀속 가능 **41건** / 세션도 NULL 2건 /
미참조 182건(워커 잡 체인 역참조로 일부 추가 가능 — 고아 정리 트랙과 교차).
⚠️ 백필된 파일은 즉시 4연산 격리가 걸리므로, 해당 파트너의 기존 회수 플로우가
자기 키로 이뤄지는지(타 키 사용 사례 없는지) 백필 전 관측 필요. D6 상신 시 포함.
