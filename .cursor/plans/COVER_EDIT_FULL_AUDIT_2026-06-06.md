...
### ✅ P1 A13 — 제본별 페이지 가드 (완료·배포, 커밋 `7453bb1`)
- useEditorStore `bindingType` 상태(기본 null) + `canAddMorePages`/`canDeletePage`에 `BINDING_CONSTRAINTS` 적용. **null=제약없음**(perfect 기본 금지로 비제본 32p 오적용 회귀 방지). 무선 min32 미만 삭제차단/중철 max64 초과 추가차단.
- useEditorContents(book 로드): `config.bindingType`→`toBindingType`(미지/미설정=null) 주입. SpreadPagePanel: 삭제가 `canDeletePage` 가드 경유(이전 우회) + 제본 안내 토스트.
- 검증: editor 빌드, useEditorStore.test 33/33. ※중철 4배수는 soft(완료시 경고) 후속. editor=Vercel 자동배포.

### ✅ P1 영역클릭 포커싱 (완료·배포·라이브검증, 커밋 `af69ac5`)
SpreadPlugin 단독(canvas-core)으로 구현 → /embed 자동작동(embed.tsx UI 배선 불필요, BLOCKER 회피). mouse:down→getRegionAtPoint(scene→content 변환)→영역 하이라이트 오버레이 토글 + spreadRegionFocus 이벤트. 가이드/라벨/오버레이 excludeFromExport(저장오염·재로드중복 근원 해소). 라이브 검증: 앞표지/뒷표지 클릭 시 하이라이트·전환 확인. ※줌·S1버튼바·S4앵커는 후속(리스크 격리).

### ⏳ 남은 P1 (설계·검증 완료, XL — 구현 대기)
- **내지PDF 표시전용 임포지션**(XL, 제품결정=표시전용): 첨부 내지 PDF를 내지 캔버스에 `excludeFromExport:true` 잠금배경(가이드)으로 표시(최종 인쇄엔 미반영). 워커 GS 다중페이지 래스터(또는 클라 pdfjs) + 게스트 인증·타임아웃·N페이지 메모리 가드.
