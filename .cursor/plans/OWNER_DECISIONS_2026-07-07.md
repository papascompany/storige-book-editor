...
## [2026-08-05 추가] D-12. 컷아웃 워커 스택 선정 (S-P2A-B 샤드1 스파이크 결과)

> 정본: `CUTOUT_WORKER_STACK_SPIKE_2026-08-05.md`. D-6a=B(워커 오프로드) 확정 후속.
> 차단 사실 3건: ①워커가 Alpine(musl)이라 onnxruntime-node 계열이 **설치는 성공하고 런타임에 터진다**(CI는 이미지를 빌드하지 않아 배포 시점 발견) ②BRIA(RMBG)는 자체호스팅 상업 사용 불가 ③단일 워커 컨테이너 공유라 추론 피크가 인쇄 파이프라인을 동반 실패시킬 수 있다(VPS 8GB).

| # | 질문 | 권고 | 결정 |
|---|---|---|---|
| D-12a | 추론 스택: C(rembg 사이드카 컨테이너) / A(워커 내장 + 베이스 alpine→glibc 교체) / F(SaaS $0.018~0.02/장) | **C** — 워커 베이스 무접촉이라 인쇄 파이프라인 회귀 위험 0, 모델 런타임 교체로 품질 실험 저비용, GPU 이전 경로 확보 | ✅ **C(rembg 사이드카) 확정** — 2026-08-05 오너 결정(권고 수용). 컨테이너 5→6개·상주 RAM 0.7~1.5GB 수용. rembg **2.0.75+ 고정**(CVE-2026-40086)·내부망 전용 바인딩·`*_custom` 세션 차단이 구현 조건. <br>**2026-08-06 프로덕션 기동 완료**(2.0.77, profile `cutout`, 포트 미노출, healthy) — 다만 D-12b 미해결로 **CUTOUT_ENABLED 는 아직 false** |
| D-12b | 기본 모델: U2Net(Apache-2.0, 데이터셋 얽힘 최소) / BEN2(MIT, 품질 우위 가능하나 DIS5K 회색지대) | **U2Net 기본 + BEN2는 20장 실측 비교 후 재상신** | ✅ **BEN2 확정(품질 우선)** — 2026-08-05 오너 결정(**권고 U2Net 기각**). ⚠️ **수용된 리스크**: BEN2 카드 원문에 DIS5K 학습이 명시돼 있어 '비상업 데이터셋 약관이 가중치까지 전이되는가'라는 법적 미해결 영역을 떠안는다(코드·가중치 라이선스는 MIT로 명확). 완화: 모델명을 코드에 하드코딩·문서화하고, 법무 검토 기회가 생기면 U2Net(Apache-2.0) 폴백 전환이 런타임 파라미터 1개로 가능하도록 설계한다 | <br>⚠️ **2026-08-06 재상신 필요 — 이 결정은 실행 불가로 판명**: ①BEN2 는 rembg 백엔드에 **없다**(있는 것은 `ben_custom` = CVE-2026-40086 경로 순회 벡터라 사용 금지) ②대체로 잡은 `birefnet-general`·`-lite` 는 프로덕션에서 **cgroup OOM**(RSS 3.13GB, mem_limit 3GiB) ③현재 유일하게 동작하는 것은 **`u2net`**(Apache-2.0, 3.6s/RSS 1.0GB) — 원 권고와 동일. 선택지: (a) u2net 확정 (b) rembg 한도 상향+워커 한도 하향으로 BiRefNet 재시도(인쇄 파이프라인 메모리 경합 리스크) (c) VPS 증설. 실측 근거: `CUTOUT_WORKER_STACK_SPIKE_2026-08-05.md` [부록 2026-08-06]<br>✅✅✅ **[2026-08-06 3차·최종] 오너 실기 QA 에서 u2net 기각 → `u2net_human_seg` 확정 + `isnet-general-use` 화이트리스트 개방**. 인물 사진에서 얼굴만 남고 정장·셔츠가 반투명하게 지워지는 것이 실측으로 확인됐다(같은 원본 1180×1178 알파 통계: **u2net 반투명 24.2% / u2net_human_seg 2.7% / isnet-general-use 18.0%**). u2net 은 salient object detection 이라 몸통을 배경으로 판정한다. 오너 결정 = **둘 다 화이트리스트에 추가**(기본 u2net_human_seg, 비인물은 클라가 isnet-general-use 명시). ⚠️ **수용된 리스크**: 두 가중치 모두 학습 데이터셋(Supervisely Person / DIS5K)의 상업 조항이 미확인·회색지대다 — D-12b 1차에서 u2net 을 고른 이유였던 '데이터셋 얽힘 최소'를 품질과 맞바꾼 결정이다. 법무 검토 기회가 생기면 재점검할 것<br><br>이전 결정(참고) — ✅✅ **재상신 결과 = (a) `u2net` 확정** — 2026-08-06 오너 결정(원 권고 수용). 근거: 유일한 동작 모델 · Apache-2.0 로 라이선스 얽힘 최소 · DIS5K 회색지대 리스크 소멸(BEN2 결정 시 수용했던 법적 미해결 영역이 함께 해소된다). **품질은 BiRefNet 대비 열위**(머리카락·반투명 경계)를 수용한 것이며, 증설·메모리 재배분 후 재검토 여지는 남는다. 배선: compose·워커 코드 기본값을 `u2net` 으로 고정 + 프로덕션 `.env` 에 `REMBG_MODEL=u2net` 명시(이중 고정), `birefnet-*` 는 화이트리스트에 남기되 문서에 OOM 경고 병기
| D-12c | F(SaaS) 선택 시에만: 고객 사진 국외이전(개인정보보호법 §28-8) 동의·DPA 설계 승인 | 자체호스팅(C·A)이면 해당 없음 | — **해당 없음**(D-12a=C 자체호스팅 확정으로 소멸) |
| D-12d | 별건: 레포에 **LICENSE 파일이 없어 imgly(AGPL-3.0) 의무를 현재도 미충족** — imgly 제거 / 상용 라이선스 구매 / LICENSE 정비 | **C안 채택 시 imgly 의존 제거가 자연 해소 경로** | ✅ **imgly 제거로 해소** — 2026-08-05 오너 결정(권고 수용). 서버 전환과 함께 canvas-core의 `@imgly/background-removal` 의존을 제거한다. ⚠️ **제거 완료 전까지 SPA `/embed` 라우트의 AGPL 노출은 지속**(임베드 IIFE는 이미 스텁 치환이라 파트너 배포물은 무관) — 샤드 2~3에서 반드시 완주할 것 |

~~보수적 기본값: 코드 변경 0~~ → **2026-08-05 전 행 기입 완료 · 샤드 2 게이트 해제.** 실행 계약: `CUTOUT_WORKER_STACK_SPIKE_2026-08-05.md` §5(12항).
