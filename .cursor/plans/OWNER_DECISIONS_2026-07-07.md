...
| # | 질문 | 권고 | 결정 |
|---|---|---|---|
| D-12a | 추론 스택: C(rembg 사이드카 컨테이너) / A(워커 내장 + 베이스 alpine→glibc 교체) / F(SaaS $0.018~0.02/장) | **C** — 워커 베이스 무접촉이라 인쇄 파이프라인 회귀 위험 0, 모델 런타임 교체로 품질 실험 저비용, GPU 이전 경로 확보 | ✅ **C(rembg 사이드카) 확정** — 2026-08-05 오너 결정(권고 수용). 컨테이너 5→6개·상주 RAM 0.7~1.5GB 수용. rembg **2.0.75+ 고정**(CVE-2026-40086)·내부망 전용 바인딩·`*_custom` 세션 차단이 구현 조건 |
| D-12b | 기본 모델: U2Net(Apache-2.0, 데이터셋 얽힘 최소) / BEN2(MIT, 품질 우위 가능하나 DIS5K 회색지대) | **U2Net 기본 + BEN2는 20장 실측 비교 후 재상신** | ✅ **BEN2 확정(품질 우선)** — 2026-08-05 오너 결정(**권고 U2Net 기각**). ⚠️ **수용된 리스크**: BEN2 카드 원문에 DIS5K 학습이 명시돼 있어 '비상업 데이터셋 약관이 가중치까지 전이되는가'라는 법적 미해결 영역을 떠안는다(코드·가중치 라이선스는 MIT로 명확). 완화: 모델명을 코드에 하드코딩·문서화하고, 법무 검토 기회가 생기면 U2Net(Apache-2.0) 폴백 전환이 런타임 파라미터 1개로 가능하도록 설계한다 |
