...
| # | 질문 | 권고 | 결정 |
|---|---|---|---|
| D-12a | 추론 스택: C(rembg 사이드카 컨테이너) / A(워커 내장 + 베이스 alpine→glibc 교체) / F(SaaS $0.018~0.02/장) | **C** — 워커 베이스 무접촉이라 인쇄 파이프라인 회귀 위험 0, 모델 런타임 교체로 품질 실험 저비용, GPU 이전 경로 확보 | ☐ |
| D-12b | 기본 모델: U2Net(Apache-2.0, 데이터셋 얽힘 최소) / BEN2(MIT, 품질 우위 가능하나 DIS5K 회색지대) | **U2Net 기본 + BEN2는 20장 실측 비교 후 재상신** | ☐ |
| D-12c | F(SaaS) 선택 시에만: 고객 사진 국외이전(개인정보보호법 §28-8) 동의·DPA 설계 승인 | 자체호스팅(C·A)이면 해당 없음 | ☐ |
| D-12d | 별건: 레포에 **LICENSE 파일이 없어 imgly(AGPL-3.0) 의무를 현재도 미충족** — imgly 제거 / 상용 라이선스 구매 / LICENSE 정비 | **C안 채택 시 imgly 의존 제거가 자연 해소 경로** | ☐ |

보수적 기본값(세션 처리): **코드 변경 0** — 오너 기입 시까지 샤드 2 착수하지 않음.
