...
| # | 질문 | 권고 | 결정 |
|---|---|---|---|
| D-6a | D-3b 확정: 컷아웃 서버 오프로드? | **A(클라 유지)** — 수요 부재·품질은 모델 문제. B는 실사용 발생+고품질 요구 시 재상신 | ☐ |
| D-6b | A 유지 시 경량화 3건(①preload lazy화 ②픽셀 캡+업로드 가드 ③dataURL→storage 치환) 착수 승인? | **승인 권고** — ①은 embed 파트너 트래픽/메모리에 즉효. 전부 additive·플래그 뒤 | ✅ **① 부분 승인** — 2026-07-15 오너가 ①(lazy화) 작업을 직접 지시(채팅). ②③은 미결 유지 |
