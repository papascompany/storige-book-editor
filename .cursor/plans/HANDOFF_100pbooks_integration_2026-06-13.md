...
| S3 | **저장계층 R2/S3 보강 + 보존정책** | ⏸ 작업 칩 `task_12f7231a` — 이 모델 스케일의 선결조건 |
| S4 | (선택) Storige 측 파일 삭제 API / 보존 cron | ⏸ S3과 함께 |
...
### 4.4 보존정책 (필수 — 용량 문제의 실제 해결)
배송완료 +N일(예 14일) 후 Storige fileId 삭제 + 컬럼 NULL. 재인쇄/CS는 **PageDoc에서 온디맨드 재생성**. PDF 장기보관 안 함.
...
## 8. 미해결/오너 결정
- Storige 측 **파일 삭제 API** 부재 → 보존정책 실행 위해 S3(작업칩)에서 삭제 엔드포인트/cron 같이 설계 필요.
- 월 주문량(런웨이 산정) — 119GB ÷ 평균 135MB ≈ 880주문. 보존정책 N일이 이 한도 안에서 안전한지 검토.
