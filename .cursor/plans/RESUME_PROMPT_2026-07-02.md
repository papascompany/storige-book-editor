...
- 배포: **editor/admin = Vercel(master push 자동)** · **api/worker = VPS 수동**(`ssh deploy@<VPS_HOST>`, `cd ~/storige && git pull && docker compose up -d --build api worker` + ⚠️ api 재배포 시 `docker compose restart nginx`(502 방지) — [[feedback_api_redeploy_nginx]])
