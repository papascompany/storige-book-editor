...
```bash
# 최신 코드 pull
ssh deploy@<VPS_HOST> 'cd ~/storige && git pull origin master'

# (P1) user_site_roles 신규 테이블 (lock 위험 없음)
ssh deploy@<VPS_HOST> 'cd ~/storige && source .env && \
...
# 확인: P1 테이블 + P2a 컬럼
ssh deploy@<VPS_HOST> 'source ~/storige/.env && docker exec storige-mariadb mariadb -ustorige -p"$DATABASE_PASSWORD" storige -e "SHOW CREATE TABLE user_site_roles\G SHOW COLUMNS FROM templates LIKE \"site_id\"; SHOW COLUMNS FROM files LIKE \"site_id\";"'
```

## 3. API 재배포 (VPS 수동 — synchronize=false 라 마이그레이션 후 배포 순서 준수)
```bash
ssh deploy@<VPS_HOST> 'cd ~/storige && docker compose build api && docker compose up -d api'
# ⚠️ nginx 옛 IP 캐싱 502 가능 → 필요 시 nginx 재시작(또는 전체 배포)
ssh deploy@<VPS_HOST> 'cd ~/storige && docker compose restart nginx'
```
...
ssh deploy@<VPS_HOST> 'source ~/storige/.env && docker exec storige-mariadb mariadb -ustorige -p"$DATABASE_PASSWORD" storige -e "DROP TABLE IF EXISTS user_site_roles;"'
...
for T in templates template_sets product_template_sets categories library_categories \
  ...
  ssh deploy@<VPS_HOST> "source ~/storige/.env && docker exec storige-mariadb mariadb -ustorige -p\"\$DATABASE_PASSWORD\" storige -e \"ALTER TABLE $T DROP COLUMN IF EXISTS site_id;\""
done
```
