...
2. **Phase 1-B** — VPS nginx `client_max_body_size` 확인 (`ssh deploy@<VPS_HOST> 'grep client_max_body_size /etc/nginx/sites-enabled/*'`)
...
### 7.1 VPS 마이그레이션 적용
```bash
scp <local>.sql deploy@<VPS_HOST>:/tmp/m.sql
ssh deploy@<VPS_HOST> 'source ~/storige/.env && \
