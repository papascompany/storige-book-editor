...
- **VPS**: `<VPS_HOST>` (Vultr Seoul, 4vCPU/8GB/160GB, Ubuntu 22.04, KST 시간대). SSH `ssh deploy@<VPS_HOST>` (key-only).
...
```bash
ssh deploy@<VPS_HOST>
ssh deploy@<VPS_HOST> 'cd ~/storige && git pull && docker compose up -d --build'
