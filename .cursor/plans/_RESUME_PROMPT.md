...
- **VPS**: `<VPS_HOST>` (Vultr Seoul, 4vCPU/8GB/160GB, Ubuntu 22.04, KST 시간대). SSH `ssh deploy@<VPS_HOST>` (key-only, NOPASSWD sudo, root 차단).
...
# DB 세션/잡
ssh deploy@<VPS_HOST> 'source ~/storige/.env && docker exec storige-mariadb mariadb -ustorige -p"$DATABASE_PASSWORD" storige -e "SELECT id, order_seqno, mode, status, completed_at FROM file_edit_sessions ORDER BY created_at DESC LIMIT 5;"'

ssh deploy@<VPS_HOST> 'docker exec storige-redis redis-cli LLEN bull:pdf-synthesis:wait'

# 워커 로그
ssh deploy@<VPS_HOST> 'docker logs --tail 200 storige-worker | grep -iE "synthesis|validate|webhook"'
```
...
# 코드 갱신 + 재배포
ssh deploy@<VPS_HOST> 'cd ~/storige && git checkout master && git pull && docker compose up -d --build'
