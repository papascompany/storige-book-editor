...
```bash
# VPS 파일 존재
ssh deploy@<VPS_HOST> 'ls -la ~/storige/storage/uploads/ | tail'
# HTTP 서빙 (nginx 직접)
curl -I https://api.papascompany.co.kr<fileUrl>   # 200 + application/pdf
# DB (/files/* 만 기록)
ssh deploy@<VPS_HOST> 'source ~/storige/.env && docker exec storige-mariadb mariadb -ustorige -p"$DATABASE_PASSWORD" storige -e "SELECT id,file_name,file_type,file_size,order_seqno FROM files ORDER BY created_at DESC LIMIT 5;"'
```
...
```bash
# 큐 소비
ssh deploy@<VPS_HOST> 'docker exec storige-redis redis-cli LLEN bull:pdf-synthesis:wait'   # 0이면 소비됨
# 잡 결과
...
# 출력 파일
ssh deploy@<VPS_HOST> 'ls -la ~/storige/storage/outputs/<jobId>/'
# 워커 로그
ssh deploy@<VPS_HOST> 'docker logs --tail 100 storige-worker | grep -iE "compose-mixed|Synthesis"'
```
