...
> editor/admin = master push 자동배포(Vercel). api/worker = 수동(`ssh deploy@<VPS_HOST> 'cd ~/storige && git pull origin master && docker compose up -d --build api worker'`). 프로덕션 실사용 중.
