...
```bash
ssh deploy@<VPS_HOST> 'cat > ~/backup.sh <<EOF
#!/usr/bin/env bash
set -e
...
```bash
ssh deploy@<VPS_HOST> '~/backup.sh && ls -lh ~/backups/'
```
...
```bash
ssh deploy@<VPS_HOST> '(crontab -l 2>/dev/null; echo "0 3 * * * /home/deploy/backup.sh >> /home/deploy/backup.log 2>&1") | crontab -'
ssh deploy@<VPS_HOST> 'crontab -l'
```
...
```bash
# rclone 설치
ssh deploy@<VPS_HOST> 'curl https://rclone.org/install.sh | sudo bash'

# config (~/.config/rclone/rclone.conf)
ssh deploy@<VPS_HOST> 'rclone config'  # interactive: type=s3, provider=Cloudflare, ...
