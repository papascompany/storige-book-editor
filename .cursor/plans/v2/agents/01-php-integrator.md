...
```bash
ssh deploy@<VPS_HOST> 'grep CORS_ORIGIN ~/storige/.env'
```
...
```bash
# API 로그
ssh deploy@<VPS_HOST> 'docker logs --tail 200 -f storige-api'

# 워커 로그
ssh deploy@<VPS_HOST> 'docker logs --tail 200 -f storige-worker'
