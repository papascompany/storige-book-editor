...
> 운영: master push → editor/admin **자동배포(Vercel)**. **API·worker 는 수동배포**(`ssh deploy@<VPS_HOST> 'cd ~/storige && git pull origin master && docker compose up -d --build api worker'`). 프로덕션 고객 실사용 중 — 빌드/테스트 후 진행.
