...
> **② 완료 모수가 사실상 1** — 한 계정이 3개월간 51세션을 반복했고 complete 15건이 전부 거기서 나왔다.

**✅ 계정 식별 완료 — 트랙 종결 (2026-09-11)**

bookmoa 가 `member_seqno` ↔ 계정 분류 대조표(회원 12명, 이메일·UUID 미포함)를 제공.
`deriveMemberSeqno` 가 UUID 결정적 해시라 역매칭이 가능했다. 당사 실값과 대조:

| member_seqno | bookmoa 분류 | 세션 | complete | editing | draft | 마지막 |
|---|---|---|---|---|---|---|
| **1607115440** | **내부(관리자 계정)** | **51** | **15** | 21 | 15 | 2026-09-02 |
| 1049737389 | 내부(편집 테스터) | 4 | 0 | 3 | 1 | 2026-06-03 |
| 615976994 | 외부 가능 | 3 | 0 | 3 | 0 | 2026-07-23 |
| 1375921922 | 외부 가능 | 2 | 0 | 2 | 0 | 2026-09-02 |
| 49385800 | 외부 가능 | 1 | 0 | 1 | 0 | 2026-08-26 |
| 715702419 | 내부(storige 도메인) | 1 | 0 | 0 | 1 | 2026-08-28 |

> **편집 완료 15건은 100% 내부 관리자 계정.** 내부 3계정 56세션 / 외부 가능 3계정 **6세션·complete 0**.
> → **R-173 개설 불요. ① 62 vs 49 교집합도 셀 필요 없어졌다**(bookmoa 가 정한 우선순위대로 ②에서 종결).

**남는 것은 결함이 아니라 사업 관찰 1건**: 외부 가능 3계정의 6세션이 **전부 `editing`** 이다 — `complete` 0, `draft` 0.
외부로 보이는 사용자는 편집기에 들어와 **저장조차 하지 않고 이탈**했다. 모수 6이라 통계적 의미는 없고, 진입 자체는 2026-09-02 까지 생존.
⚠️ "외부 가능" 3계정이 실제 외부 고객인지는 bookmoa 만 안다(그쪽도 이메일 도메인 외 근거 없음). 그것까지 내부면 **외부 사용자 편집기 진입 0건** = 오픈 전 상태 그대로다. **오너 판단 영역으로 bookmoa 오너에게 이관됨.**

> **양측 종결 확인(2026-09-11)**: bookmoa `836acb2` — R-173 불요 · 62 vs 49 교집합 미실시 · 당사 62개 값 전달 불요.
> **D5 통지 1건에서 시작해 존재하지 않는 장애 2건(합성 PDF 다운로드 파손 · 편집 완료 유실)을 각각 로그와 DB 로 걷어냈다.**
> 한쪽 관측면만 있었으면 둘 다 긴급 트랙이 됐다 — 파트너 실측과 당사 관측면을 교차한 것이 이 트랙의 방법론적 성과다.
...
**이번 트랙에서 이 함정이 4번 나왔고 5번째가 미수에 그쳤다.** 어느 쪽이 몇 번인지는 적지 않는다 —
한쪽은 주문 도메인을, 한쪽은 세션 스키마를 본다. 자기 테이블이라 먼저 본 것이지 누가 더 잘 아는 게 아니다.
재사용 가치는 귀속이 아니라 규칙에 있다:

- **jsonb `?` 는 값이 아니라 키를 센다** (`{sessionId: null}` 을 명시적으로 박는 코드가 있으면 키-존재 판정은 전부 무효)
- **`IS NOT NULL` 은 NOT NULL 컬럼에서 무의미하다** (항상-NULL 도 항상-채움도 판정력이 없다)
- **컬럼을 판정축으로 쓰기 전에 nullability·센티널(`=0`·음수)·전 모집단 분포를 먼저 본다**
- **폴백 기본값을 완료 지문으로 읽지 마라** — `status:'edited'` 는 완료가 아니라 **주문 생성 시점 기본값**이었다(`AppContext` 가 미상 status 를 그 값으로 폴백). 상태 문자열은 **"그 값을 누가 언제 쓰는가" 를 코드에서 확인한다. 이름이 의미를 보장하지 않는다**
- **내 도메인이 아닌 테이블의 축을 남에게 제안할 때는, 상대가 스키마를 확인해 주기 전까지 결론을 쓰지 않는다** (`order_seqno` 건이 정확히 그 경우)

> 양측 원장에 동일 규칙으로 고정됨 — bookmoa 종결 커밋 `836acb2`.
