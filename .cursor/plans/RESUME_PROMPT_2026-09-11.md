...
| `claude/100p_books` | CI `node-version: 22` · engines `>=20` | 2.11.2 | 127 | ✅ 정합(node@22 로드 성공). 단 `.nvmrc` 부재 |
...
**실제로 남는 것 2건(오늘 작업과 무관, 미조치 — 오너 판단)**
- `100p_books`: `.nvmrc` 부재 → 기본 node 로 무심코 돌리면 canvas 에서 터진다. CI(22)와는 정합하니 결함이 아니라 **개발 편의 공백**
