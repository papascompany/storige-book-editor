...
4. **배포 게이트**: VPS `deploy@<VPS_HOST>`(키 로드 `ssh-add -l`), `cd ~/storige && git pull origin master`. API 코드 변경이라 `docker compose up -d --build api` + `docker compose restart nginx`(IP캐시 502 방지). nginx.conf 변경 반영 위해 nginx 재시작 필수. 마이그레이션 없음(이번엔 스키마 변경 없을 듯).
