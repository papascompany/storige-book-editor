...
#### 8-13-1. 신규 결속 12건 전수 대조 — "목록은 스테일해진다" 의 확정 증거 (2026-09-24)

bookmoa 가 §8-13 확인 요청에 답하며 **신규 결속 12건 id** 를 줬다(그쪽 `order_asset_claims` kind=file 58 → **70**, 장바구니 3건은 불변).
**35건의 성격**: 09-21 08:30Z 이후 bookmoa 주문 7건 중 Storige 파일 보유 3건 → **결속 12건(보존 필수)** + 나머지 ~23건은 **오너 주문 테스트 업로드**(미주문·장바구니 미참조, 정식 오픈 전 테스트 트래픽).

**전수 대조 결과**(당사 DB, 09-24)

| 항목 | 실측 |
|---|---|
| 12개 id 존재 | **12/12** · soft-deleted 0 · 09-21 목록과 중복 0 |
| site 스탬프 | **bookmoa 12 · NULL-site 0** ✅ (R-149 이후 업로드라 전부 스탬프됨) |
| `order_seqno` 채워진 것 | **0건** — 진술과 일치(bookmoa 는 Storige 에 보내지 않는다) |
| 용량·시각 | 455.55MB · 2026-09-22 04:13~06:45Z |
| (a)안 판정 대상 | **12/12** · 30일 경과 **0**(2일) → 진입 예정 **≈2026-10-22** |

- 📌 **보존 필수 집합: 70(주문) + 3(장바구니) = 73건**(09-21 61건 → 09-24 73건)
- ✅ **백필 대상은 여전히 25건**(신규 12건은 전부 스탬프돼 NULL-site 하드삭제 결함 §8-8 사정권 밖). 백필 범위 재산정 불필요
- 🚨 **이것이 (a)안 기각의 확정 근거다**: 3일 만에 **결속 12건과 미주문 23건이 함께 생겼고, 당사는 그 둘을 구분할 수 없었다.**
  구분은 bookmoa 가 알려줘야만 가능하다 = **정적 제외 목록은 파트너의 수동 통지 주기에 종속**된다. grace 30일보다 통지가 늦으면 실주문이 지워진다
