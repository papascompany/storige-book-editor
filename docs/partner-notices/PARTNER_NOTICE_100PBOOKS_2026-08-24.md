# Storige 연동 변경 안내 (2026-08-24) — 100p Books 팀
...
귀사 연동(유형 1: 자체 편집기 + PDF 저장/검증 오프로드)이 사용하는 경로는 이번 변경 대상이 아닙니다:

- `X-API-Key` 기반 업로드/외부조회(`/files/*`, `/edit-sessions/external`)·워커 잡 경로 — **기존 site 격리 규칙 그대로**, 변경 없음
