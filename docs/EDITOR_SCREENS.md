# 에디터 화면 기획서

> 고객용 캔버스 에디터 화면

---

## 1. 전체 레이아웃

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│  [헤더] 작업명                   [실행취소][다시실행]  [불러오기] [편집완료]  [설정]  │
├──────┬───────────────┬────────────────────────────────────────────┬─────────────────┤
│      │               │                                            │                 │
│ 툴바 │   사이드바    │              캔버스 영역                    │   사이드패널    │
│      │  (FeatureSB   │                                            │   (페이지/      │
│      │   또는        │                                            │    객체 목록)   │
│      │   ControlBar) │                                            │                 │
│      │               │                                            │                 │
│      │               │                                            │                 │
│      │               │                                            │                 │
│      │               │                                            │                 │
│      │               │                                            │                 │
└──────┴───────────────┴────────────────────────────────────────────┴─────────────────┘
```

### 상세 레이아웃

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                     │
│   나의 새로운 작업 1        [↩️][↪️][💻]  [📂 불러오기] [✅ 편집완료] [⚙️] [❓]        │
│                                                                                     │
├──────┬───────────────┬────────────────────────────────────────────┬─────────────────┤
│      │               │                                            │                 │
│  📤  │               │                                            │   ┌─ 페이지 ──┐ │
│업로드│               │                                            │   │           │ │
│      │               │                                            │   │ [+] 추가  │ │
│  ✂️  │               │     ┌─────────────────────────────┐        │   │           │ │
│모양컷│               │     │                             │        │   │ ┌───┐     │ │
│      │               │     │                             │        │   │ │ 1 │앞날개│ │
│  📋  │  [사이드바    │     │                             │        │   │ └───┘     │ │
│템플릿│   콘텐츠]     │     │       작업 영역             │        │   │ ┌───┐     │ │
│      │               │     │                             │        │   │ │ 2 │앞표지│ │
│  🖼️  │               │     │                             │        │   │ └───┘     │ │
│이미지│               │     │                             │        │   │ ┌───┐     │ │
│      │               │     │                             │        │   │ │ 3 │책등 │ │
│  T   │               │     │                             │        │   │ └───┘     │ │
│텍스트│               │     └─────────────────────────────┘        │   │  ...      │ │
│      │               │                                            │   │           │ │
│  ⬡   │               │                                            │   └───────────┘ │
│ 요소 │               │                                            │                 │
│      │               │                    [◀️ 이전] [3/14] [다음 ▶️]│   ┌─ 객체 ───┐ │
│  🎨  │               │                                            │   │           │ │
│ 배경 │               │                                            │   │ [객체1]   │ │
│      │               │                                            │   │ [객체2]   │ │
│  🔲  │               │                                            │   │  ...      │ │
│프레임│               │                                            │   │           │ │
│      │               │                                            │   └───────────┘ │
│  📱  │               │                                            │                 │
│QR코드│               │                                            │                 │
│      │               │                                            │                 │
│  🔧  │               │                                            │                 │
│편집툴│               │                                            │                 │
│      │               │                                            │                 │
└──────┴───────────────┴────────────────────────────────────────────┴─────────────────┘
```

---

## 2. 헤더 영역

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                     │
│   ┌──────────────────────────┐                                                      │
│   │ 나의 새로운 작업 1       │  (클릭하여 이름 편집)                                │
│   └──────────────────────────┘                                                      │
│                                                                                     │
│   [↩️ 실행취소] [↪️ 다시실행] [💻 미리보기]                                          │
│                                                                                     │
│                              [📂 불러오기] [✅ 편집완료] [⚙️ 설정] [❓ 도움말]       │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

### 저장 상태 표시

```
정상:   나의 새로운 작업 1  ✓ 저장됨
저장중: 나의 새로운 작업 1  ◐ 저장 중...
오류:   나의 새로운 작업 1  ⚠️ 저장 실패 [재시도]
```

---

## 3. 툴바 (좌측)

```
┌──────┐
│      │
│  📤  │  업로드 - 이미지 업로드
│      │
├──────┤
│  ✂️  │  모양컷 - 클리핑 마스크
│      │
├──────┤
│  📋  │  템플릿 - 템플릿 교체
│      │
├──────┤
│  🖼️  │  이미지 - 스톡 이미지
│      │
├──────┤
│  T   │  텍스트 - 텍스트 추가  [선택됨]
│      │       ─────────────────
├──────┤
│  ⬡   │  요소 - 도형/아이콘
│      │
├──────┤
│  🎨  │  배경 - 배경 설정
│      │
├──────┤
│  🔲  │  프레임 - 이미지 프레임
│      │
├──────┤
│  📱  │  QR/바코드
│      │
├──────┤
│  🔧  │  편집도구
│      │
└──────┘
```

---

### 저장 흐름 매트릭스 (admin / 고객 분리)

| 진입 경로 | URL | 저장 시 호출 | 저장 대상 | 영향 범위 |
|---|---|---|---|---|
| Admin → 템플릿관리 → "편집" | `/template?templateId=…` (TemplateEditorView) | `PATCH /templates/:id` | `templates.canvas_data` 1건 | 단일 템플릿 |
| **Admin → 템플릿셋관리 → "템플릿셋 수정"** | `/?templateSetId=…&adminEdit=templateSet` (EditorView) | `PATCH /templates/:id` × N | 각 페이지 templateId 의 `canvas_data` (중복 제거 후) | 템플릿셋 전체 페이지의 디폴트 디자인 |
| Admin → 템플릿셋관리 → "설정" | `/template-sets/:id` (TemplateSetForm) | `PATCH /template-sets/:id` | 템플릿셋 메타 (이름·판형·페이지 구성·도구 메뉴) | 메타만, 캔버스 무관 |
| **고객 (PHP/bookmoa iframe)** | `embed.tsx` 번들 | `PATCH /edit-sessions/:id` + `filesApi.upload` + `complete()` | `edit_sessions` + 업로드 PDF | 본인 주문 작품 |
| 고객 (standalone 새 탭) | `/?templateSetId=…` (EditorView, adminEdit 없음) | (현재 미연결) | — | "독립 실행 모드" 토스트만 표시 |

**원칙**:
- "Admin 권한 + URL 파라미터" 두 조건이 모두 맞아야 admin 저장 분기 활성. customer JWT 토큰으로는 `adminEdit=templateSet` 을 보내도 `useIsAdmin()===false` 라 분기되지 않음.
- "템플릿셋 수정" 모드: 같은 templateId 가 여러 페이지에 반복되면 첫 번째 페이지의 캔버스만 PATCH. 페이지마다 다른 디자인을 원하면 admin 이 "설정" 에서 페이지마다 다른 templateId 를 미리 추가해 둬야 함.
- 고객 흐름은 `adminEdit` 파라미터를 인식하지 못하므로 admin 이 입힌 디폴트 디자인을 자동으로 베이스로 받게 됨 (templates.canvas_data 가 갱신되었으므로).

### Admin 라벨 (혼동 방지)

| 메뉴 | 라벨 | 아이콘 | 동작 |
|---|---|---|---|
| 템플릿 관리 | **편집** | ✏ | 단일 템플릿 캔버스 편집 → templates 갱신 |
| 템플릿셋 관리 | **템플릿셋 수정** | 🖥 | 모든 페이지 캔버스 편집 → 각 templates 갱신 |
| 템플릿셋 관리 | **설정** | ✏ | 템플릿셋 메타 수정 (form) — 캔버스 무관 |

이전 버전의 "에디터" 라벨은 모호해 폐기. "수정 vs 설정" 으로 캔버스 vs 메타 의미를 구분.

### Editor 헤더 — 모드별 UI 분리 (사용자 vs 관리자 이질감 정리)

같은 EditorView 가 모드별로 다른 헤더를 노출해 혼동을 차단:

| 영역 | 고객 (PHP/embed) | Admin "템플릿셋 수정" | Admin standalone (legacy) |
|---|---|---|---|
| 좌측 저장 인디케이터 | `AutoSaveIndicator` (실시간) | **"수동 저장 모드" 뱃지** (amber) | "수동 저장 모드" 뱃지 |
| 우측 "불러오기" | 표시 (내 작업 라이브러리) | **숨김** (templateSet 자체를 편집 중) | 표시 |
| 우측 액션 1 | "내 작업에 저장" | **"저장"** (창 유지, ⌘S) | (없음) |
| 우측 액션 2 | **"편집완료"** (PHP onFinish 콜백) | **"저장 후 닫기"** (저장+window.close) | "편집완료" (saveWorkForAdmin) |
| 저장 가드 | 자동저장 + 명시 저장 | **`window.confirm` 다이얼로그**: "이 템플릿셋의 모든 페이지 디자인을 갱신합니다. ..." | 없음 |
| 상단 안내 배너 | 없음 | **amber 배너**: 수정 의미 + 영향 범위 + 액션 안내 | 없음 |

**원칙**:
- "Admin 템플릿셋 수정" 모드는 한 번의 저장이 같은 templateSetId 로 진입하는 모든 사용자에게 영향을 주므로 **자동저장 절대 금지** + **명시적 confirm 가드** + **시각적 amber 강조** 3중 보호.
- 자동저장 인디케이터를 그대로 두면 "저장됨" 거짓 표시 위험 → 모드별로 인디케이터 자체를 교체.
- "저장" 과 "저장 후 닫기" 분리: 중간 저장 가능 + 작업 마무리 명확.

**확인 다이얼로그 (`window.confirm`) 표시 내용**:
```
이 템플릿셋의 모든 페이지 디자인을 갱신합니다.

• 영향 페이지: <N>개
• PATCH 대상 templates: <M>개 (중복 templateId 제거)
• 갱신 후 같은 templateSetId 로 진입하는 모든 사용자에게 새 디자인이 보입니다.

저장 후 [계속 편집할 수 있습니다 | 창을 닫습니다]. 진행하시겠습니까?
```

### 디폴트 진입 시 자동 로드되는 샘플 템플릿셋

URL 파라미터 없이 `/` 로 진입하면 (productId/contentId/templateSetId/editMode 모두 없음) 자동으로 **샘플 8×8 inch 책 템플릿셋(24p)** 이 로드됩니다 — 표지(스프레드)부터 순서대로 편집 가능.

| 항목 | 값 |
|---|---|
| 템플릿셋 ID | `sample-8x8-book-24p` |
| 이름 | 샘플 8×8 inch 책 (24p) |
| 판형 | 203.2 × 203.2 mm (8 × 8 inch) |
| 에디터 모드 | `book` (책모드 — 스프레드 편집) |
| 구성 | 표지 스프레드 1 + 내지 24 |
| pageCountRange | `[8, 24, 48]` (사용자가 페이지 수 조정 가능) |

표지 스프레드(`sample-spread-cover-8x8`):
- 408.1 × 203.2 mm (앞표지 + 책등 1.7mm + 뒤표지)
- 24p 무선제본 모조지 80g 기준 책등 폭 = `(24/2) × 0.10 + 0.5 = 1.7 mm`
- 화면에 들어가면 SpreadPlugin 이 가이드 + 라벨 자동 그림

내지 (`sample-page-8x8`):
- 203.2 × 203.2 mm
- 같은 templateId 가 24번 반복 (각 페이지가 독립 캔버스로 로드되어 별도 편집)

**시드 데이터**: [`apps/api/migrations/20260508_seed_sample_template_set.sql`](../apps/api/migrations/20260508_seed_sample_template_set.sql) — `INSERT ... ON DUPLICATE KEY UPDATE` 로 idempotent.

**디폴트 변경 / 비활성화**:
- 환경변수 `VITE_DEFAULT_TEMPLATE_SET_ID=other-id` → 다른 템플릿셋으로 교체
- `VITE_DEFAULT_TEMPLATE_SET_ID=none` (또는 `disabled`, 빈 문자열) → 기존 100×100mm 빈 캔버스 디폴트로 복원
- 구현: [`apps/editor/src/constants/defaultTemplateSet.ts`](../apps/editor/src/constants/defaultTemplateSet.ts)

---

### 도구 메뉴 노출 화이트리스트 (템플릿셋별)

**개요**

좌측 ToolBar의 도구 메뉴(업로드/모양컷/템플릿/이미지/텍스트/요소/배경/프레임/QR·바코드/편집도구/AI)는 템플릿셋(상품) 단위로 노출 여부를 제어할 수 있습니다.

**저장 위치**: `template_sets.enabled_menus` (JSON 배열, nullable)

| 값 | 의미 |
|---|---|
| `null` (기본) | 모든 메뉴 노출 — legacy 호환 |
| `[ ... ]` 배열 | 화이트리스트 — 배열에 포함된 키만 노출, 순서는 ToolBar 순서를 따름 |
| `[]` 빈 배열 | 모든 도구 메뉴 숨김 |

**설정 위치 (Admin)**
- 템플릿셋 편집 화면 → "에디터 도구 메뉴" 섹션
- "도구 메뉴 노출 직접 설정" 토글
  - **끔**: 모든 메뉴 노출 (`enabledMenus: null` 저장)
  - **켬**: 체크박스 그룹에서 노출할 메뉴 선택
- 빠른 프리셋 버튼: 전체 선택 / 전체 해제 / 업로드만

**적용 흐름 (Editor)**
1. URL `?templateSetId=...` 로 에디터 진입
2. `loadTemplateSetEditor()` 가 templateSet 조회 → `enabledMenus` 추출
3. `useSettingsStore.setEnabledMenus()` 로 store 에 저장
4. `ToolBar` 가 `useSettingsStore.enabledMenus` 를 구독해 자동 필터링
5. **`editMode === true`** (admin 편집 미리보기) 에서는 화이트리스트 무시 → 전체 노출

**도구 메뉴 키 (`EditorMenuKey`)**

| 키 | 라벨 | 빌드 플래그 |
|---|---|---|
| `UPLOAD` | 업로드 | `VITE_ENABLE_UPLOAD_MENU` |
| `CLIPPING` | 모양컷 | `VITE_ENABLE_IMAGE_PROCESSING` (OpenCV) |
| `TEMPLATE` | 템플릿 | `VITE_ENABLE_TEMPLATE_MENU` |
| `IMAGE` | 이미지 | — |
| `TEXT` | 텍스트 | — |
| `SHAPE` | 요소 | — |
| `BACKGROUND` | 배경 | — |
| `FRAME` | 프레임 | `VITE_ENABLE_FRAME_MENU` |
| `SMART_CODE` | QR/바코드 | `VITE_ENABLE_SMART_CODE_MENU` |
| `EDIT` | 편집도구 | `VITE_ENABLE_IMAGE_PROCESSING` |
| `AI` | AI | `VITE_ENABLE_AI_PANEL` |

> **빌드 플래그가 꺼진 메뉴는 화이트리스트에 포함되어 있어도 노출되지 않습니다.** 두 조건이 모두 통과해야 노출됩니다.

**예시 시나리오**

```jsonc
// 동화책 — 프레임/QR 안 씀
{ "enabledMenus": ["UPLOAD","CLIPPING","TEMPLATE","IMAGE","TEXT","SHAPE","BACKGROUND","EDIT","AI"] }

// 전단지 — AI/모양컷 안 씀
{ "enabledMenus": ["UPLOAD","TEMPLATE","IMAGE","TEXT","SHAPE","BACKGROUND","FRAME","SMART_CODE","EDIT"] }

// 업로드만 (단순 PDF 입고용)
{ "enabledMenus": ["UPLOAD"] }
```

**새 도구 추가 절차**

1. `packages/types/src/index.ts` 의 `EditorMenuKey` 와 `EDITOR_MENU_DEFS` 에 항목 추가
2. `apps/editor/src/components/editor/ToolBar.tsx` 의 `ALL_MENUS` 에 메뉴 정의 추가 (아이콘/onTap 등)
3. `pnpm --filter @storige/types build` 후 admin/editor 빌드
4. Admin 의 체크박스 그룹은 자동 갱신 (별도 코드 변경 불필요)

---

## 4. 사이드바 (FeatureSidebar vs ControlBar)

### 4.1 FeatureSidebar (도구 선택 시)

**텍스트 도구 선택 시:**

```
┌─────────────────────┐
│  텍스트         [X] │
├─────────────────────┤
│                     │
│  텍스트             │
│  ┌─────────────────┐│
│  │ [+] 텍스트 추가 ││
│  └─────────────────┘│
│                     │
│  ─────────────────  │
│                     │
│  추천 스타일        │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │제목A│ │제목B│   │
│  └─────┘ └─────┘   │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │본문A│ │본문B│   │
│  └─────┘ └─────┘   │
│                     │
└─────────────────────┘
```

**이미지 도구 선택 시:**

```
┌─────────────────────┐
│  이미지         [X] │
├─────────────────────┤
│                     │
│  [검색어 입력...]   │
│                     │
│  카테고리           │
│  [전체▼]            │
│                     │
│  ─────────────────  │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │img1 │ │img2 │   │
│  │     │ │     │   │
│  └─────┘ └─────┘   │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │img3 │ │img4 │   │
│  │     │ │     │   │
│  └─────┘ └─────┘   │
│                     │
│      더 보기 ▼      │
│                     │
└─────────────────────┘
```

**템플릿 도구 선택 시:**

```
┌─────────────────────┐
│  템플릿         [X] │
├─────────────────────┤
│                     │
│  [템플릿셋] [낱장]  │
│  ═════════         │
│                     │
│  ─ 템플릿셋 교체 ─  │
│                     │
│  ┌─────────────────┐│
│  │                 ││
│  │  A4 기본 세트   ││
│  │   (현재 적용)   ││
│  │                 ││
│  └─────────────────┘│
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │세트A│ │세트B│   │
│  │     │ │     │   │
│  └─────┘ └─────┘   │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │세트C│ │세트D│   │
│  │     │ │     │   │
│  └─────┘ └─────┘   │
│                     │
└─────────────────────┘
```

**템플릿 낱장 교체 탭:**

```
┌─────────────────────┐
│  템플릿         [X] │
├─────────────────────┤
│                     │
│  [템플릿셋] [낱장]  │
│            ═══════  │
│                     │
│  현재 페이지: 내지  │
│  (page 타입만 표시) │
│                     │
│  ─────────────────  │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │내지A│ │내지B│   │
│  │(현재)│ │     │   │
│  └─────┘ └─────┘   │
│                     │
│  ┌─────┐ ┌─────┐   │
│  │     │ │     │   │
│  │내지C│ │내지D│   │
│  │     │ │     │   │
│  └─────┘ └─────┘   │
│                     │
└─────────────────────┘
```

---

### 객체 선택 핸들

선택된 객체의 외곽에 표시되는 핸들 형태:

```
        ┌──[──]──┐                  ← 상단 변: 가로 캡슐 (세로 스케일)
        │        │
   ●────│        │────●              ← 4 코너: 원형 (자유 리사이즈)
   │    │  객체   │    │
   [    │        │    ]              ← 좌우 변: 세로 캡슐 (가로 스케일)
   │    │        │    │
   ●────│        │────●
        │        │
        └──[──]──┘
            │
            │  (점선 연결)
            ⊙                       ← 회전 핸들: 원형 + 회전 아이콘 (객체 아래)
```

| 핸들 | 모양 | 동작 | 비고 |
|---|---|---|---|
| 4 코너 (`tl/tr/bl/br`) | 원형 (Ø12px, 터치 ×1.33) | 자유 비율 리사이즈 | 텍스트는 코너만 활성 |
| 좌·우 변 (`ml/mr`) | 세로 캡슐 (7×22px) | 가로 스케일 / Shift+드래그=스큐 | 텍스트/`lockUniScaling`은 숨김 |
| 상·하 변 (`mt/mb`) | 가로 캡슐 (22×7px) | 세로 스케일 / Shift+드래그=스큐 | 텍스트/`lockUniScaling`은 숨김 |
| 회전 (`mtr`) | 원형 + 회전 화살표 아이콘 | 회전 (15° 스냅) | **객체 아래 36px** — 텍스트 가독성 보호 |

- 보더/핸들 색상은 light=`rgba(39,99,138,0.66)` / dark=`rgba(142,207,69,0.7)` 으로 테마 자동 동기화.
- 회전 화살표는 객체가 회전해도 **항상 화면 기준으로 표시** (직관성).
- 터치(`pointer:coarse`) 환경에서는 핸들 크기/hit-area 자동 확대 (16px / 36px 대각).
- 구현: [`packages/canvas-core/src/plugins/ControlsPlugin.ts`](../packages/canvas-core/src/plugins/ControlsPlugin.ts) — `fabric.Object.prototype.controls` 의 각 컨트롤 `render` 커스터마이즈.

### 4.2 ControlBar (객체 선택 시)

**텍스트 객체 선택 시:**

```
┌─────────────────────┐
│  ┌───┐              │
│  │ T │  텍스트      │
│  └───┘  [🔒][👁️][🗑️]│
├─────────────────────┤
│                     │
│  ▼ 크기             │
│  ┌────────┐┌──────┐ │
│  │W: 24.5 ││H:11.9│ │
│  └────────┘└──────┘ │
│                     │
│  ▼ 속성             │
│  ┌─────────────────┐│
│  │ Noto Sans KR  ▼ ││
│  └─────────────────┘│
│                     │
│  ┌──────┐ [B] [U]   │
│  │ 29.8 │           │
│  └──────┘           │
│                     │
│  ┌──────┐ ┌──────┐  │
│  │줄간격││ 자간  │  │
│  │ 1.16 ││  0   │  │
│  └──────┘ └──────┘  │
│                     │
│  [좌] [중앙] [우]   │
│                     │
│  ▼ 채우기           │
│  ┌────┐             │
│  │████│ #000000     │
│  └────┘   100%      │
│                     │
└─────────────────────┘
```

**이미지 객체 선택 시:**

```
┌─────────────────────┐
│  ┌───┐              │
│  │🖼️ │  이미지      │
│  └───┘  [🔒][👁️][🗑️]│
├─────────────────────┤
│                     │
│  ▼ 크기             │
│  ┌────────┐┌──────┐ │
│  │W: 100  ││H: 80 │ │
│  └────────┘└──────┘ │
│                     │
│  [🔗 비율 유지]     │
│                     │
│  ▼ 위치             │
│  ┌────────┐┌──────┐ │
│  │X: 50   ││Y: 30 │ │
│  └────────┘└──────┘ │
│                     │
│  ▼ 필터             │
│  ┌─────────────────┐│
│  │밝기      [====] ││
│  │대비      [====] ││
│  │채도      [====] ││
│  └─────────────────┘│
│                     │
│  [이미지 교체]      │
│  [자르기]           │
│                     │
└─────────────────────┘
```

**다중 선택 시:**

```
┌─────────────────────┐
│  ┌───┐              │
│  │≡≡ │  3개의 아이템│
│  └───┘  [🔒][👁️][🗑️]│
├─────────────────────┤
│                     │
│  [🔗 그룹화]        │
│                     │
│  ─────────────────  │
│                     │
│  ▼ 정렬             │
│                     │
│  [⬅️][➡️][⬆️][⬇️]    │
│   좌  우  상  하    │
│                     │
│  [↔️][↕️]            │
│  가로 세로          │
│  정렬 정렬          │
│                     │
│  ▼ 분배             │
│                     │
│  [↔️ 가로 분배]     │
│  [↕️ 세로 분배]     │
│                     │
└─────────────────────┘
```

---

## 5. 캔버스 영역

```
┌────────────────────────────────────────────────────────────────────────┐
│                                                                        │
│    │-100  │-80   │-60   │-40   │-20   │ 0    │ 20   │ 40   │ 60      │
│  ──┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼──────┼───────  │
│    │                                                                   │
│ -40│      ┌─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┐           │
│    │      ╎ ┌───────────────────────────────────────────┐ ╎           │
│ -20│      ╎ │                                           │ ╎           │
│    │      ╎ │                                           │ ╎           │
│  0 │      ╎ │              작업 영역                    │ ╎           │
│    │      ╎ │                                           │ ╎           │
│ 20 │      ╎ │          ┌─────────────┐                  │ ╎           │
│    │      ╎ │          │   TEXT      │◀── 선택된 객체   │ ╎           │
│ 40 │      ╎ │          └─────────────┘                  │ ╎           │
│    │      ╎ │                                           │ ╎           │
│ 60 │      ╎ └───────────────────────────────────────────┘ ╎           │
│    │      └ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┘           │
│    │       ▲                                                           │
│    │       블리드 영역 (재단선)                                        │
│  ──┼───────────────────────────────────────────────────────────────── │
│                                                                        │
│                                                                        │
│                        [◀️ 이전] [3 / 14] [다음 ▶️]                     │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```

### 가이드라인 표시

```
범례:
─ ─ ─  빨간 점선: 블리드(재단선) - 잘려나갈 수 있는 영역
───────  실선: 페이지 경계
· · · ·  파란 점선: 안전 영역 - 중요 내용은 이 안에
```

### 워크스페이스 자동 중앙 정렬

캔버스 영역의 폭이 변하면(사이드 메뉴 토글/사이드바 드래그/객체 선택 시 ControlBar 등장/창 리사이즈) 편집중인 페이지(workspace)가 자동으로 새 영역의 중앙으로 재배치됩니다.

| 상황 | 동작 |
|---|---|
| 현재 줌에서 페이지가 새 영역에 들어감 | **줌 유지** + 중앙으로만 재배치 (`WorkspacePlugin.setCenterPointOf`) |
| 페이지가 새 영역을 넘어감 (확대 상태에서 영역이 더 좁아진 경우) | **자동 맞춤**(`setZoomAuto`)으로 페이지 전체가 다시 보이도록 스케일 + 중앙 정렬 |

- 5% 여백을 둬서(`PADDING = 0.95`) 경계선에서의 미세 진동(자동맞춤 ↔ 중앙이동 반복) 방지.
- 첫 마운트는 `WorkspacePlugin.reset()` 의 `setZoomAuto()` 가 처리하므로 이 자동 재정렬은 그 이후 발생한 영역 변화에만 적용.
- 구현 위치: [`apps/editor/src/views/EditorView.tsx`](../apps/editor/src/views/EditorView.tsx) — ResizeObserver `apply()`.

---

## 6. 사이드패널 (우측)

### 6.1 페이지 패널

```
┌─────────────────────┐
│  페이지         [+] │
├─────────────────────┤
│                     │
│  ┌─────────────────┐│
│  │ ┌───┐           ││
│  │ │ 1 │ 앞날개    ││
│  │ └───┘   wing    ││
│  └─────────────────┘│
│                     │
│  ┌─────────────────┐│
│  │ ┌───┐           ││
│  │ │ 2 │ 앞표지    ││
│  │ └───┘   cover   ││
│  │         [필수]  ││
│  └─────────────────┘│
│                     │
│  ┌─────────────────┐│
│  │ ┌───┐           ││
│  │ │ 3 │ 책등      ││ ◀── 현재 페이지 (강조)
│  │ └───┘   spine   ││
│  └═════════════════┘│
│                     │
│  ┌─────────────────┐│
│  │ ┌───┐           ││
│  │ │ 4 │ 내지 1    ││
│  │ └───┘   page    ││
│  │         [≡] 🗑️  ││ ◀── 드래그 핸들 + 삭제
│  └─────────────────┘│
│                     │
│       ...           │
│                     │
│  ┌─────────────────┐│
│  │ ┌───┐           ││
│  │ │14 │ 뒷날개    ││
│  │ └───┘   wing    ││
│  └─────────────────┘│
│                     │
│  ─────────────────  │
│  내지: 10 / 10~40   │
│  ─────────────────  │
│                     │
└─────────────────────┘
```

### 페이지 타입별 표시

```
wing  타입: 순서 고정, 삭제 불가 (deleteable에 따름)
cover 타입: 순서 고정, 필수 표시
spine 타입: 순서 고정
page  타입: 드래그 순서 변경 가능, 삭제 가능 (deleteable에 따름)
```

### 6.2 객체 패널

```
┌─────────────────────┐
│  객체               │
├─────────────────────┤
│                     │
│  ┌─────────────────┐│
│  │ [T] 제목 텍스트 ││ ◀── 선택된 객체
│  │     [👁️] [🔒]   ││
│  └═════════════════┘│
│                     │
│  ┌─────────────────┐│
│  │ [🖼️] 로고 이미지││
│  │     [👁️] [🔒]   ││
│  └─────────────────┘│
│                     │
│  ┌─────────────────┐│
│  │ [T] 본문 텍스트 ││
│  │     [👁️] [🔓]   ││
│  └─────────────────┘│
│                     │
│  ─────────────────  │
│                     │
│  객체가 없습니다.   │
│  (빈 페이지)        │
│                     │
└─────────────────────┘
```

---

## 7. 모달/다이얼로그

### 7.1 템플릿셋 교체 확인

```
┌─────────────────────────────────────────────────────────────────┐
│  템플릿셋 교체                                             [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "A4 프리미엄 세트"로 교체하시겠습니까?                         │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ ⚠️ 주의                                                 │    │
│  │                                                         │    │
│  │ • 템플릿 디자인이 변경됩니다.                           │    │
│  │ • 직접 추가한 텍스트, 이미지, 도형은 유지됩니다.        │    │
│  │ • 기존 템플릿의 요소는 새 템플릿으로 교체됩니다.        │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│                                      [취소]  [교체]             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.2 페이지 삭제 확인

```
┌─────────────────────────────────────────────────────────────────┐
│  페이지 삭제                                               [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "내지 5" 페이지를 삭제하시겠습니까?                            │
│                                                                 │
│  • 이 페이지의 모든 편집 내용이 삭제됩니다.                     │
│  • 이 작업은 되돌릴 수 없습니다.                                │
│                                                                 │
│                                      [취소]  [삭제]             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.3 내지 수량 제한 알림

```
┌─────────────────────────────────────────────────────────────────┐
│  페이지 추가 불가                                          [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ⚠️ 내지 수량이 최대 한도에 도달했습니다.                       │
│                                                                 │
│  현재: 40페이지 / 최대: 40페이지                                │
│                                                                 │
│  더 많은 페이지가 필요하시면 고객센터로 문의해 주세요.          │
│                                                                 │
│                                             [확인]              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.4 편집 완료 확인

```
┌─────────────────────────────────────────────────────────────────┐
│  편집 완료                                                 [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  편집을 완료하고 검토 요청하시겠습니까?                         │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ 📋 검토 전 확인사항                                     │    │
│  │                                                         │    │
│  │ ☑ 모든 페이지를 확인했습니다.                          │    │
│  │ ☑ 텍스트 오탈자를 검토했습니다.                        │    │
│  │ ☑ 이미지 해상도를 확인했습니다.                        │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                 │
│  ⚠️ 완료 후에는 직접 수정이 불가능합니다.                       │
│                                                                 │
│                                   [계속 편집]  [완료]           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.5 저장 실패 알림

```
┌─────────────────────────────────────────────────────────────────┐
│  저장 실패                                                 [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ⚠️ 네트워크 오류로 저장에 실패했습니다.                        │
│                                                                 │
│  작업 내용은 임시 저장되었습니다.                               │
│  네트워크 연결 확인 후 다시 시도해 주세요.                      │
│                                                                 │
│                                  [나중에]  [다시 시도]          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 7.6 편집 잠금 알림 (동시 편집 방지)

```
┌─────────────────────────────────────────────────────────────────┐
│  편집 중인 사용자가 있습니다                               [X]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  🔒 현재 다른 사용자가 편집 중입니다.                           │
│                                                                 │
│  편집자: 관리자 (admin@store.com)                               │
│  시작 시간: 2024-01-15 14:30                                    │
│                                                                 │
│  읽기 전용으로 열거나 나중에 다시 시도해 주세요.                │
│                                                                 │
│                              [읽기 전용으로 열기]  [닫기]       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 8. 책등 편집 화면

**템플릿셋에 spine 타입이 있을 때 표시:**

```
┌────────────────────────────────────────────────────────────────────────┐
│                                                                        │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                                                                 │   │
│  │  ┌────────┬────────────────────────────────────────┬────────┐  │   │
│  │  │        │                                        │        │  │   │
│  │  │        │                                        │        │  │   │
│  │  │        │                                        │        │  │   │
│  │  │  뒤    │               책  등                   │  앞    │  │   │
│  │  │  표    │             (spine)                    │  표    │  │   │
│  │  │  지    │                                        │  지    │  │   │
│  │  │        │              ┌────┐                    │        │  │   │
│  │  │        │              │제목│                    │        │  │   │
│  │  │        │              └────┘                    │        │  │   │
│  │  │        │                                        │        │  │   │
│  │  │        │                                        │        │  │   │
│  │  └────────┴────────────────────────────────────────┴────────┘  │   │
│  │                                                                 │   │
│  │            ◀──── 책등 폭: 8.5mm ────▶                          │   │
│  │                                                                 │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                        │
│  ┌─ 책등 설정 ─────────────────────────────────────────────────────┐   │
│  │                                                                 │   │
│  │  페이지 수: [  40  ] 페이지                                     │   │
│  │                                                                 │   │
│  │  종이 종류: [모조지 80g        ▼]                               │   │
│  │                                                                 │   │
│  │  제본 방식: [무선제본 (Perfect) ▼]                              │   │
│  │                                                                 │   │
│  │  ─────────────────────────────────────────────────────────────  │   │
│  │                                                                 │   │
│  │  계산된 책등 폭: 8.5mm                                          │   │
│  │                                                                 │   │
│  │  ⚠️ 책등 폭이 좁습니다. 텍스트 배치에 주의하세요.               │   │
│  │                                                                 │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```

---

## 9. 읽기 전용 모드

**검토 중/완료 상태 또는 다른 사용자가 편집 중일 때:**

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│  ┌──────────────────────────────────────────────────────────────────────────────┐   │
│  │ 🔒 읽기 전용 모드 - 현재 검토 중입니다. 편집이 필요하면 관리자에게 문의하세요. │   │
│  └──────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                     │
│   나의 작업 1 (읽기 전용)                                      [닫기]               │
│                                                                                     │
├──────┬───────────────┬────────────────────────────────────────────┬─────────────────┤
│      │               │                                            │                 │
│      │  [모든 도구   │                                            │                 │
│      │   비활성화]   │         (편집 불가)                        │                 │
│      │               │                                            │                 │
│      │               │                                            │                 │
└──────┴───────────────┴────────────────────────────────────────────┴─────────────────┘
```

---

## 10. 로딩/상태 화면

### 10.1 초기 로딩

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                     │
│                                                                                     │
│                                                                                     │
│                                                                                     │
│                                   ◐                                                 │
│                                                                                     │
│                            에디터를 초기화하는 중...                                │
│                                                                                     │
│                                                                                     │
│                                                                                     │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

### 10.2 템플릿 로딩 실패

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                     │
│                                                                                     │
│                                                                                     │
│                                   ⚠️                                                │
│                                                                                     │
│                         템플릿을 불러오는데 실패했습니다.                           │
│                                                                                     │
│                              [다시 시도]  [뒤로 가기]                               │
│                                                                                     │
│                                                                                     │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
```

---

## 11. 화면 흐름도

```
                              ┌─────────────────┐
                              │   쇼핑몰에서    │
                              │  상품/템플릿셋  │
                              │     선택        │
                              └────────┬────────┘
                                       │
                                       ▼
                              ┌─────────────────┐
                              │   에디터 진입   │
                              │  (템플릿 로드)  │
                              └────────┬────────┘
                                       │
           ┌───────────────────────────┼───────────────────────────┐
           │                           │                           │
           ▼                           ▼                           ▼
   ┌───────────────┐          ┌───────────────┐          ┌───────────────┐
   │   페이지      │          │   요소 편집   │          │   템플릿      │
   │   관리        │          │  (캔버스)     │          │   교체        │
   │               │          │               │          │               │
   │ • 페이지 선택 │          │ • 텍스트 추가 │          │ • 템플릿셋    │
   │ • 순서 변경   │          │ • 이미지 추가 │          │   전체 교체   │
   │ • 추가/삭제   │          │ • 도형 추가   │          │ • 낱장 교체   │
   └───────┬───────┘          └───────┬───────┘          └───────┬───────┘
           │                           │                           │
           └───────────────────────────┼───────────────────────────┘
                                       │
                                       ▼
                              ┌─────────────────┐
                              │   자동 저장     │
                              │   (상시)        │
                              └────────┬────────┘
                                       │
                                       ▼
                              ┌─────────────────┐
                              │   편집 완료     │
                              │  (검토 요청)    │
                              └────────┬────────┘
                                       │
                                       ▼
                              ┌─────────────────┐
                              │   상태 변경     │
                              │  draft → review │
                              └─────────────────┘
```

---

## §10 인쇄 워크플로우 v1 — 화면 흐름 (2026-05-19)

> 자세한 사양: `docs/EDITOR.md` §13 + `docs/PHP_NOTICE_2026-05-19_pdf_attach_endpapers.md`

### §10.1 게스트 진입 + Floating Workflow Controls

```
[비로그인 사용자 진입]
    │
    ▼
┌──────────────────────────────────────────────┐
│  EditorView (apps/editor/src/views/EditorView.tsx) │
│  ┌────────────────────────────────────────┐  │
│  │ EditorHeader                           │  │
│  │ ─────────────────────────────────────  │  │
│  │ [ToolBar] [Canvas + PagePanel]         │  │
│  │                              ┌───────┐ │  │
│  │                              │ 🏷 레더 │ │  │  ← floating
│  │                              │  커버  │ │  │     (coverEditable=false 시)
│  │                              ├───────┤ │  │
│  │                              │ 📄 면지 │ │  │  ← floating
│  │                              │ 앞2/뒤1 │ │  │     (endpaperConfig 있을 때)
│  │                              ├───────┤ │  │
│  │                              │📎PDF첨부│ │  │  ← floating
│  │                              └───────┘ │  │     (book mode + 게스트/회원 세션)
│  └────────────────────────────────────────┘  │
│  EditorWorkflowControls (Phase 5-D)          │
│   - useGuestStore.ensureGuestSession() 호출  │
│   - templateSet 로드 → 분기                  │
└──────────────────────────────────────────────┘
```

### §10.2 페이지 네비 — 표지 + 면지 + 내지 (개념)

```
PagePanel:
  ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐ ┌─────┐
  │ 표지 │ │앞면지│ │앞면지│ │ 내지 │ │ 내지 │ │ 내지 │ │뒷면지│
  │spread│ │  1  │ │  2  │ │  1  │ │  2  │ │  3  │ │  1  │
  └─────┘ └─────┘ └─────┘ └─────┘ └─────┘ └─────┘ └─────┘
   editable  editable readonly editable editable editable editable
   or readonly                  ↑
   (coverEditable)              내지 PDF 첨부 모드면 모두 readonly
                                (결정 3-3 배타)
```

### §10.3 ContentPdfAttachModal 흐름

```
[📎 PDF 첨부 버튼 클릭]
    │
    ▼
┌────────────────────────────────────────┐
│  ContentPdfAttachModal                 │
│  1. 파일 선택 (PDF, 50MB)              │
│  2. POST /api/storage/upload-public    │
│  3. POST /api/worker-jobs/validate     │
│  4. 30s 폴링 → 검증 결과               │
│      ├─ completed → 페이지 확장 체크   │
│      ├─ fixable → 사용자 확인          │
│      └─ failed → 첨부 거부 (결정 3-4) │
│  5. (확장 필요 시) 자동확장 모달       │
│      "PDF 24p, 내지 16p — 자동확장?"  │
│  6. PATCH /edit-sessions/guest/:id     │
│      contentPdfFileId 저장             │
└────────────────────────────────────────┘
```

### §10.4 GuestAuthPromptModal (편집완료 시)

```
[편집완료 클릭 + 게스트 + 비로그인]
    │
    ▼
┌────────────────────────────────────────┐
│  GuestAuthPromptModal                  │
│  ┌──────────────────────────────────┐  │
│  │ 편집 완료 — 작업 저장            │  │
│  │ 게스트 작업은 24시간 후 자동삭제 │  │
│  │  [로그인 / 회원가입]             │  │  → postMessage editor.needAuth
│  │  [게스트로 계속 (24h 후 삭제)]   │  │  → onContinueAsGuest 콜백
│  └──────────────────────────────────┘  │
└────────────────────────────────────────┘
    │
    ▼ (로그인 완료 후)
window.__storigeMigrateNow()
    │
    ▼
POST /api/edit-sessions/guest/migrate { guestToken }
    │
    ▼
{ migratedCount, sessionIds[] } — 회원 흡수 완료
```

### §10.5 MyWorksView `/my-works`

```
┌──────────────────────────────────────────────┐
│  내 작업                                     │
│  ─────────────────────────────────────────   │
│  • 템플릿셋: ts-001 — A4 무선제본            │
│    상태: complete · 업데이트: 2026-05-19     │
│    📎 PDF 첨부됨 (24p)         [재편집]      │
│  • 세션 abc12345…                            │
│    상태: editing · 업데이트: 2026-05-18      │
│                                  [재편집]    │
│  • ...                                       │
└──────────────────────────────────────────────┘
```

비로그인 시: "로그인이 필요합니다" + 편집기 복귀 버튼.
