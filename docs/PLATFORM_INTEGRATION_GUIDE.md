...
> 🔴 **흡수 후 이미 열린 편집기에서 in-place 재완료는 지원하지 않습니다 (2026-09-24 명시).** 3번 흡수가 성공한 뒤 **열어 둔 편집기에서 「편집완료」를 다시 누르게 하면 실패합니다.**
> 편집기는 게스트 여부를 **자기 메모리의 세션 상태(`guestToken`)** 로 판정하는데, 흡수는 서버에서 `guestToken` 을 지우므로 편집기의 저장·완료가 `403 NOT_A_GUEST_SESSION` → `editor.error`(`SAVE_FAILED`)로 끝납니다. 흡수 후에는 자동저장도 같은 이유로 매 주기 실패합니다.
> 호스트→편집기 명령(`getState`·`saveNow`·`setBackGuard`)에는 흡수 사실이나 회원 토큰을 열린 편집기에 전달하는 명령이 **없습니다.** 따라서:
> - 흡수 성공 후에는 **반드시 4번(같은 `sessionId` 를 회원 accessToken 으로 `/embed` 재오픈)** 으로 가세요. 재오픈된 편집기가 회원 세션으로 로드되어 정상 완료 경로를 탑니다.
