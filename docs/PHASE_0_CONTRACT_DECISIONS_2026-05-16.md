...
## 6.5. Phase A-2 결정사항 (2026-05-16 추가)

Phase 3 어댑터 구현 직전, Codex 와 Claude 양쪽 작업 산출물 점검 중 발견된 5건의 의문점을 사용자가 모두 확정. **공통 원칙: 기존 코드 / PHP 가이드가 단일 진실이며, 기존 PHP 사이트는 단 한 줄도 변경하지 않는다.**

| # | 의문점 | 결정 | 권위 출처 | 기존 PHP 영향 |
|---|---|---|---|---|
| A2-1 | `validation.fixable` 처리 — `failed` 합치기 vs 별도 status | **별도 status `fixable`** (failed 아님). 자동 보정 가능, 사용자 확인 필요 상태 | `docs/SYSTEM_INTEGRATION_OVERVIEW.md` 225~259 | **없음** — PHP 측 webhook 핸들러는 이미 이벤트별 분기 |
| A2-2 | `siteId` URL 파라미터 — 필수 vs 선택 vs 미전달 | **선택(optional)**. URL 파라미터는 UX 분기·로깅 보조용. **권한 권위는 JWT payload 의 `siteId`**. URL 위조 가능하므로 권한 결정에 사용 금지 | `apps/api/src/sites/entities/site.entity.ts` + `apps/editor/src/embed.tsx` | **없음** — PHP 는 URL siteId 미전달, JWT 자동 폴백으로 동작 |
| A2-3 | `orderSeqno` 생성 시점 — 장바구니 단계 임시 ID vs 결제 후 실제 | **실제 주문번호만**. 결제 후 발급된 순간에 편집기 진입 또는 합성 요청. **장바구니 단계 cart item.storige.orderSeqno = `null`** | `docs/PHP_INTEGRATION_FINAL_v3.md` 880~881 | **없음** — PHP 는 이미 결제 직후 발급 패턴 |
| A2-4 | `editor.complete` payload 형식 — top-level flat vs `files.*` 중첩 | **`files.*` 중첩** (envelope 한 겹 포함). `payload.files.coverFileId` 형태로 접근 | `apps/editor/src/embed.tsx` L113~129 `EditorResult` 인터페이스 | **없음** — PHP 측은 이미 이 형식으로 수신 중. Codex 어댑터만 정렬 |
| A2-5 | webhook 수신부 저장소 — KV/file 별도 vs Supabase 일원화 | **Supabase 일원화 (Option A)**. 사용자 운영 Supabase 가 항상 ON 으로 확정. `SUPABASE_SERVICE_ROLE_KEY` 로 `app_config.p4-orders` 갱신 | `bookmoa-mobile/src/lib/storage.js` 96~111 의 `sLoad/sSave` 추상화 | **없음** — bookmoa-mobile 한정 결정. PHP 는 자체 DB 사용 |

### 후속 문서 반영 (본 결정사항 적용 위치)

| 결정 | 반영 문서 / 섹션 |
|---|---|
| A2-1 (fixable) | `Bookmoa_platform_Plan.md` Phase 6 §6.1 status enum (5→6 확장) |
| A2-2 (siteId 선택) | `docs/PLATFORM_INTEGRATION_v1.md` §4.1, §4.2 + `Bookmoa_platform_Plan.md` Phase 5.4 표 |
| A2-3 (orderSeqno null) | `Bookmoa_platform_Plan.md` Phase 6 cart item.storige 스키마 + Phase 5.4 표 |
| A2-4 (files.* 중첩) | `Bookmoa_platform_Plan.md` Phase 5.2.1 EditorResult 스키마 + `docs/PLATFORM_INTEGRATION_v1.md` §4.3 |
| A2-5 (Supabase 일원화) | `Bookmoa_platform_Plan.md` Phase 7 webhook.js 흐름 + `SUPABASE_SERVICE_ROLE_KEY` env |

### 영향 분석 요약

- **PHP 측 운영**: 5건 모두 영향 0. 기존 키, 기존 URL, 기존 webhook 핸들러 그대로 작동.
- **Storige 측 코드**: 변경 없음. 기존 `embed.tsx` / API DTO / sites 엔티티가 이미 단일 진실의 권위 출처이며, 본 결정은 그 권위를 명문화한 것.
- **bookmoa-mobile (Codex 작업 영역)**: 어댑터 구현 시 본 결정 그대로 적용. 추가 마이그레이션·설정 변경 없음.
