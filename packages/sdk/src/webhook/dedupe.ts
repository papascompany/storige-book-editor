 ...
 *
 * ============================================================================
 * 🚨 dedupe 키는 **서명 밖**이다 — 멱등은 인증 통제가 아니다
 * ============================================================================
 * 서명 data 는 `${t}.${identifier}:${event}:${timestamp}` 이고, identifier 가
 * jobId/sessionId 로 정해지는 페이로드에서는 **delivery uid 가 서명에 들어가지
 * 않는다**(uid 가 서명에 쓰이는 건 jobId·sessionId 가 둘 다 없어 uid 로 폴백할
 * 때뿐이다). 그런데 dedupe 는 `X-Storige-Delivery` **헤더값을 직독**한다.
 *
 * 귀결: 유효 서명 1건을 캡처한 공격자는 **헤더의 uid 만 바꿔** 같은 서명을
 * tolerance 창(기본 300초) 안에서 **반복 재생**할 수 있다(단락은 uid 가 다르니
 * 걸리지 않고, 서명은 uid 를 안 덮으니 그대로 유효하다). 실증: 핸들러 5회 실행.
 *
 * → **멱등은 신뢰성 통제(서버 재시도 접기)이지 인증 통제가 아니다.**
 *   부작용이 있는 핸들러는 **자체 도메인 멱등을 병행**하라 — 주문 uid·jobId+event
 *   조합 등 **본인 도메인 키**로 "이미 처리했는가"를 판정하고, 상태 전이는
 *   조건부 갱신(CAS)으로 하라. SDK 의 uid 단락은 그 위에 얹는 1차 필터다.
 *   (tolerance 창을 좁힐수록 재생 가능 시간이 준다 — 기본 300초.)
 *
 * 근본 해법은 발신 서명 data 에 uid 를 포함하는 것이지만 그것은 **발신 계약 변경**
 * 이라 SDK 범위 밖이다(발신·수신 동시 배포 필요 — 서버 트랙).
 */
