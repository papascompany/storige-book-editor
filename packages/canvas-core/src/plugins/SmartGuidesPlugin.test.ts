...
describe('SmartGuidesPlugin — §6-3 스냅 토글 게이트', () => {
  it('setObjectSnapEnabled(false) 면 객체 스냅·가이드 모두 스킵', () => {
    const workspace = makeObj({ id: 'workspace', left: 0, top: 0, width: 1000, height: 1000 })
    const candidate = makeObj({ id: 'c1', left: 100, top: 100, width: 80, height: 80 })
    const moving = makeObj({ id: 'm1', left: 105, top: 500, width: 50, height: 50 })
    const objects = [workspace, candidate, moving]
    const { canvas, plugin } = setup(objects)
    ;(plugin as unknown as { setObjectSnapEnabled: (v: boolean) => void }).setObjectSnapEnabled(false)

    fireMoving(canvas, moving)

    expect(moving.left).toBe(105) // 스냅 없음(105→100 안 함)
    const visible = objects.filter((o) => o.extensionType === 'guideline' && o.visible)
    expect(visible.length).toBe(0)

    // 다시 켜면 스냅 복귀
    ;(plugin as unknown as { setObjectSnapEnabled: (v: boolean) => void }).setObjectSnapEnabled(true)
    fireMoving(canvas, moving)
    expect(moving.left).toBe(100)
  })

  it('setAngleSnapEnabled(false) 면 회전 각도 라운딩 안 함', () => {
    const workspace = makeObj({ id: 'workspace', left: 0, top: 0, width: 1000, height: 1000 })
    const target = makeObj({ id: 'm1', left: 100, top: 100, width: 50, height: 50, angle: 43.4 })
    const { canvas, plugin } = setup([workspace, target])
    ;(plugin as unknown as { setAngleSnapEnabled: (v: boolean) => void }).setAngleSnapEnabled(false)

    fireRotating(canvas, target)
    expect(target.angle).toBe(43.4) // 45 로 스냅 안 함

    ;(plugin as unknown as { setAngleSnapEnabled: (v: boolean) => void }).setAngleSnapEnabled(true)
    fireRotating(canvas, target)
    expect(target.angle).toBe(45)
  })

  it('setCenterYieldEnabled(false) 면 중앙 근처도 객체 스냅 적용(데드존 없음)', () => {
    const workspace = makeObj({ id: 'workspace', left: 0, top: 0, width: 1000, height: 1000 })
    // 워크스페이스 중앙(500,500). 이동 객체 중심 x≈505(양보 반경 8px 내), 후보 centerX=502
    const yieldCandidate = makeObj({ id: 'c3', left: 462, top: 700, width: 80, height: 80 }) // centerX=502
    const moving = makeObj({ id: 'm1', left: 480, top: 175, width: 50, height: 50 }) // center=(505,200)
    const objects = [workspace, yieldCandidate, moving]
    const { canvas, plugin } = setup(objects)

    // 기본(양보 ON): 중앙 8px 내라 x축 스냅 양보 → 505 유지
    fireMoving(canvas, moving)
    expect((moving.getCenterPoint as () => { x: number })().x).toBe(505)

    // 양보 OFF: 중앙 근처여도 객체 스냅 적용 → 502 로 스냅(데드존 없음)
    moving.left = 480
    ;(plugin as unknown as { setCenterYieldEnabled: (v: boolean) => void }).setCenterYieldEnabled(false)
    fireMoving(canvas, moving)
    expect((moving.getCenterPoint as () => { x: number })().x).toBe(502)
  })
})
